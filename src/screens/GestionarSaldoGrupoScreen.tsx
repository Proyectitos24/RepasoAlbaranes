...
async function tableColumns(db: any, table: string): Promise<Set<string>> {
  const info = (await db.getAllAsync(`PRAGMA table_info(${table});`)) as Array<{
    name: string;
  }>;
...
export default function GestionarSaldoGrupoScreen({ route }: any) {
  ...
  const aplicarDelta = async (codigo: string, delta: number) => {
    ...
    try {
      ...
      if (next === 0) {
        await db.runAsync(
          `DELETE FROM saldo_global WHERE grupo = ? AND codigo = ?;`,
          [grupo, codigo],
        );
      } else if (rows.length) {
        await db.runAsync(
  ...
  const filtered = useMemo(() => {
    ...
    if (filtro === "faltas")
      list = list.filter((it) => Number(it.saldo ?? 0) < 0);
    if (filtro === "sobras")
      list = list.filter((it) => Number(it.saldo ?? 0) > 0);
    ...
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <View style={styles.headerActions}>
          ...
          <Pressable
            style={[styles.filterBtn, filtro !== "all" && styles.filterBtnOn]}
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.signRow}>
              <Pressable
                style={[
                  styles.signBtn,
                  tempSign === "falta" && styles.signBtnOnRed,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.signTxt,
                    tempSign === "falta" && styles.signTxtOn,
                  ]}
                >
                  Falta
                </Text>
              ...
              <Pressable
                style={[
                  styles.signBtn,
                  tempSign === "sobra" && styles.signBtnOnGreen,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.signTxt,
                    tempSign === "sobra" && styles.signTxtOn,
                  ]}
                >
                  Sobra
                </Text>
...
const styles = StyleSheet.create({
  ...
  searchClear: { position: "absolute", right: 10, top: "50%", marginTop: -10 },
