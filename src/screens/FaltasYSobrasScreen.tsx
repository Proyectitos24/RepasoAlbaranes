...
import { borrarSaldoGrupo, listarResumenPorGrupos, listarSaldoPorGrupo } from '../database/saldo';
...
export default function FaltasYSobrasScreen({ navigation }: any) {
  const [resumen, setResumen] = useState<Array<{ grupo: string; items: number; faltan: number; sobran: number }>>([]);
  const [grupoSel, setGrupoSel] = useState<string>(CARPETAS[0].key);
  const [items, setItems] = useState<Array<{ codigo: string; descripcion: string | null; saldo: number }>>([]);
  ...
  const labelGrupo = useMemo(() => CARPETAS.find(c => c.key === grupoSel)?.label ?? grupoSel, [grupoSel]);
  ...
  const resumenMap = useMemo(() => {
    const m = new Map<string, { items: number; faltan: number; sobran: number }>();
    for (const r of resumen) m.set(r.grupo, { items: r.items, faltan: r.faltan, sobran: r.sobran });
    return m;
  }, [resumen]);
  ...
  const Card = ({ k, label }: { k: string; label: string }) => {
    ...
    return (
      <Pressable style={[styles.folderCard, active && styles.folderCardActive]} onPress={() => setGrupoSel(k)}>
        <Text style={styles.folderTitle}>{label}</Text>
        <Text style={styles.folderMeta}>Ítems: {r.items}</Text>
        <Text style={styles.folderMeta}>Faltan: {r.faltan}  |  Sobran: {r.sobran}</Text>
      </Pressable>
    );
  };
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Pressable
              ...
              onPress={() => navigation.navigate('DetalleSaldo', { grupo: grupoSel, codigo: item.codigo, descripcion: item.descripcion })}
            >
      ...
      <Modal transparent visible={pinOpen} animationType="fade" onRequestClose={() => setPinOpen(false)}>
        <View style={styles.backdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSub}>Introduce el PIN para borrar esta carpeta.</Text>
...
const styles = StyleSheet.create({
  ...
  folderCard: { flex: 1, backgroundColor: '#fff', padding: 12, borderRadius: 12, borderWidth: 2, borderColor: '#eee' },
  ...
  itemCard: { backgroundColor: '#fff', padding: 12, borderRadius: 12, marginBottom: 10, borderWidth: 2 },
